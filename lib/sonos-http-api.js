...
function HttpAPI(discovery, settings) {
  ...
  this.setSocketIO = (io) => {
    ...
    discovery.on('transport-state', (player) => {
      invokeWebhook('transport-state', player, _io);
    });

    discovery.on('topology-change', (topology) => {
      invokeWebhook('topology-change', topology, _io);
    });

    discovery.on('volume-change', (volumeChange) => {
      invokeWebhook('volume-change', volumeChange, _io);
    });

    discovery.on('mute-change', (muteChange) => {
      invokeWebhook('mute-change', muteChange, _io);
    });
  };
  ...
  function invokeWebhook(type, data, io) {
   let typeName = 'type';
    let dataName = 'data';
    ...
    if (settings.webhook === 'socket.io') {
      ...
      io.sockets.emit(`event:${type}`, jsonBody);
    } else {
      request({
        method: 'POST',
